"""llama.cpp server adapter using aiohttp for OpenAI-compatible API."""
import logging
...
class LlamaCppAdapter(LLMProvider):
    """Adapter for llama.cpp server's OpenAI-compatible /v1 endpoints."""

    def __init__(self, base_url: str = "http://localhost:8080", model: str = "llama"):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.request(method, f"{self.base_url}{path}", json=json) as resp:
                    if resp.status >= 400:
                        logger.warning(f"llama.cpp {method} {path} failed: {resp.status}")
                        return None
                    return await resp.json()
        except aiohttp.ClientError as e:
            logger.warning(f"llama.cpp connection error: {e}")
            return None
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(f"{self.base_url}/v1/chat/completions", json=payload) as resp:
                    if resp.status >= 400:
                        logger.warning(f"llama.cpp stream failed: {resp.status}")
                        return
                    buffer = ""
            ...
            logger.warning(f"llama.cpp stream error: {e}")
